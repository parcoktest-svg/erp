...
export default function UomsView() {
  ...
  const columns: ColumnsType<UomRow> = useMemo(
    () => [
          ...
          <Space>
            <Button
              ...
              onClick={() => {
                setEditing(r)
                setOpen(true)
                form.setFieldsValue({ code: r.code, name: r.name, active: r.active })
              }}
            >
              Edit
            </Button>
            <Popconfirm
              ...
              onConfirm={async () => {
                if (!companyId) return
                try {
                  await masterDataApi.deleteUom(companyId, r.id)
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Card>
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          ...
          <Table<UomRow> rowKey="id" loading={loading} columns={columns} dataSource={filtered} pagination={{ pageSize: 10 }} />
        </Space>
      </Card>
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            ...
            if (editing) {
              await masterDataApi.updateUom(companyId, editing.id, values)
              message.success('Updated')
            } else {
              await masterDataApi.createUom(companyId, values)
