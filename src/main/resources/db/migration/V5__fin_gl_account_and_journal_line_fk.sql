...
-- Idempotent: add gl_account_id column if missing
SET @gl_col_exists := (
  SELECT COUNT(*)
...
-- Idempotent: add index if missing
SET @gl_idx_exists := (
  SELECT COUNT(*)
...
SET @sql_add_gl_fk := IF(
  @gl_fk_exists = 0,
  'ALTER TABLE fin_journal_line ADD CONSTRAINT fk_fin_journal_line_gl_account FOREIGN KEY (gl_account_id) REFERENCES fin_gl_account(id)',
  'SELECT 1'
);
